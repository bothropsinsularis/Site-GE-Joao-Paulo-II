...
$stmt = $pdo->prepare("SELECT * FROM images WHERE categoria = ?");
...
<nav class="navbar navbar-expand-lg py-3 fixed-top custom-navbar">
    ...
    <h1>Imagens da Categoria <?= htmlspecialchars($categoria) ?></h1><br>
    <div class="imagens">
        <?php foreach ($images as $image): ?>
            <a href="visualizar.php?id=<?= htmlspecialchars($image['id']) ?>">
                <img src="uploads/<?= htmlspecialchars($image['filename']) ?>" width="400px" height="400px" alt="<?= htmlspecialchars($image['title']) ?>">
            </a>
        <?php endforeach; ?>
    </div><br>
