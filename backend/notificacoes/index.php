...
$sql = "SELECT * FROM notificacoes WHERE userid = '$userid' ORDER BY data DESC";
...
<nav class="navbar navbar-expand-lg py-3 custom-navbar">
    ...
    <div class="content">
        <h1>Notificações</h1><br>
        <ul>
            <?php while ($row = $result->fetch_assoc()): ?>
                <li>
                    <?php 
                    // Definir a URL com base no tipo de notificação
                    if ($row['tipo'] == 'resposta') {
                        // Redireciona para a resposta específica dentro do tópico
                        $url = "../forum/topico.php?id=" . $row['item_id'];
                    } else {
                        // Para comentários, apenas redireciona para o tópico
                        $url = "../forum/topico.php?id=" . $row['item_id'];
                    }
                    ?>
                    <a class="notifica" href="<?php echo $url; ?>">
                        <?php echo htmlspecialchars($row['mensagem']); ?>
                    </a><br>
                    <small>(<?php echo $row['data']; ?>)</small>
                    <form action="excluir_notificacao.php" method="POST" style="display:inline;">
                        <input type="hidden" name="notificacao_id" value="<?php echo $row['id']; ?>">
                        <button class="btn-custom" id="notify" type="submit">Excluir</button><br>
                    </form>
                </li>
            <?php endwhile; ?>
        </ul>
    ...
    <footer class="footer text-center text-lg-start d-flex w-100 justify-content-between align-items-center fixed-bottom">
